...
ifeq ($(BUILD_TEXT), 1)
  ...
  CXX_SRCS := $(filter-out src/$(PROJECT)/text.cpp, $(shell find src/ -name "*.cpp"))
endif

# TOOL_SRCS are the source files for the tool binaries
ifeq ($(BUILD_TEXT), 1)
  TOOL_SRCS := $(shell find src/ -name "*.cpp")
else
  TOOL_SRCS := $(filter-out src/tools/aesthetic_feature_extractor.cpp, $(shell find src/ -name "*.cpp"))
endif
...
#endif
INCLUDE_DIRS += $(BUILD_INCLUDE_DIR) ./include
LIBRARIES += avcodec avdevice avfilter avformat avutil swscale \
			opencv_core opencv_highgui opencv_calib3d opencv_features2d opencv_flann opencv_imgproc opencv_imgcodecs  opencv_video opencv_photo 
#postproc swresample
...
ifeq ($(DEBUG), 1)
	...
	COMMON_FLAGS += -std=c++0x -DNDEBUG -O2 -Wdeprecated-declarations
endif
